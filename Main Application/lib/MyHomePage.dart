...
class UploaderState extends State<Uploader> {
  ...
  Future<void> _startUpload() async {
    ...
    setState(() {
      ...
      _uploadTask =
          (_storage.ref()).child(storageFilePath).putFile(widget.file);
  ...
  Future<String> genSeqImageName() async {
    ...
    await dbRefSeq.once().then((DataSnapshot snapshot) {
      tempstr = snapshot.value
          .toString(); //Number is retuned in this format: {NUM: 27}
      uploadsNumStr = tempstr.substring(
          5, tempstr.length - 1); //Use substring to get number only
  ...
  Future<void> saveReferenceToRealtimeDatabase(
    ...
    databaseReference
        .child(databasePath)
        .push()
        .set({'imageName': name, 'imageURL': url});
  }
...
class _HomePageState extends State<HomePage> {
  ...
  _openGallery(
      BuildContext
          context) async //Tells app to wait untill user selects image, however long that takes them to complete that action.
  {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          backgroundColor: Colors.blue[400], title: Text('Custom Emojiboard') //
          ),
